...
genome = WorkflowNfcasereports.create_file_channel(params.homeology_genome)
...
workflow HRD_ONENESS_TWONESS {
    ...
    EVENT_HOMEOLOGY(events_output)
    homeology_for_merge = EVENT_HOMEOLOGY.out.homeology.map { meta, homeology -> [meta.patient, homeology] }
    homeology_stats_for_merge = EVENT_HOMEOLOGY.out.homeology_stats.map { meta, homeology_stats -> [meta.patient, homeology_stats] }
    ...
    ONENESS_TWONESS(oneness_twoness_input, model)
