...
workflow NFGOS {
    ...
    // Use the samplesheet channels from TOOLS.out so that the
    // --overwrite_subsequent transformation (if enabled) is reflected.
    // When overwrite_subsequent is false these are identical to SETUP.out.inputs
    // and SETUP.out.inputs_unlaned respectively.
    inputs         = TOOLS.out.inputs
    inputs_unlaned = TOOLS.out.inputs_unlaned
