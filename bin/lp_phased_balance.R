...
`%|||%` = function(x,y) {
    if (is.null(x) || NROW(x) == 0L) y else x
}
...
hets.gr = dt2gr(hets.dt[, .(seqnames, start, end, strand = rep("*", length.out = NROW(hets.dt)), alt = alt.count.t, ref = ref.count.t)])
hets.gr = JaBbA:::seqlevels_to_ncbi(hets.gr)
...
nodes.ov.hets = gr.findoverlaps(spl,
                                dt2gr(hets.dt[, .(seqnames, start, end, strand = rep("*", length.out = NROW(hets.dt)))]),
                                return.type = "data.table")

nodes.ov.hets$query.id = rep_len(nodes.ov.hets$query.id %|||% NA_integer_, NROW(nodes.ov.hets))
nodes.ov.hets$subject.id = rep_len(nodes.ov.hets$subject.id %|||% NA_integer_, NROW(nodes.ov.hets))
