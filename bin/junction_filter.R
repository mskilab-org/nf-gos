...
if (file.exists(opt$pon)) {
    ...
    within_pon = suppressWarnings(suppressMessages(gGnome::fra.overlaps(this_filt, pon_object, pad = opt$padding, return.as.data.table = TRUE)))
    filter_these = unique(within_pon[["ra1.ix"]])
    filter_these = filter_these[!is.na(filter_these)]  # Remove NAs
    nr = NROW(filter_these)
    message("Unique SVs that overlap with the PON: ", nr)
    return_this_filtered_somatic = this_filt
    if (nr > 0) {
    ...
    return_this_filtered_somatic = gr.fix(return_this_filtered_somatic, gnomAD_object)
    gnomAD_object = gr.fix(gnomAD_object, return_this_filtered_somatic)
    within_gnomAD = suppressWarnings(suppressMessages(gGnome::fra.overlaps(return_this_filtered_somatic, gnomAD_object, pad = opt$padding, return.as.data.table = TRUE)))
    ...
    nr = NROW(filter_these_gnomAD)
    message("Remaining SVs that overlap with the gnomAD PON: ", nr)
    return_this_filt = return_this_filtered_somatic
    if (nr > 0) {
