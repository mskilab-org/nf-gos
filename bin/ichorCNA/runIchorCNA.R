...
print("saving cmd args")
saveRDS(opt, file = paste0(opt$outDir, "/cmd.args.rds"))
...
if (opt$genomeStyle == "NCBI") {
  seqlevels(si) <- sub("^chr", "", seqlevels(si))
}
seqinfo <- si
...
## add small fudge factor to purity when estimate is 0
iter <- hmmResults.cor$results$iter
for (s in 1:numSamples) {
  if (hmmResults.cor$results$n[s, iter] == 1.0) {
    hmmResults.cor$results$n[s, iter] <- 0.99
  }
}
